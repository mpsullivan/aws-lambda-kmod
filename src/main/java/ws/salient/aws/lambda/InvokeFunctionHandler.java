...
public class InvokeFunctionHandler implements WorkItemHandler {

    private static final Logger log = LoggerFactory.getLogger(InvokeFunctionHandler.class);

    private final AWSLambda lambda;
    private ObjectMapper json;

    @Inject
    public InvokeFunctionHandler(AWSLambda lambda, ObjectMapper json) {
        this.lambda = lambda;
        this.json = json;
    }
    ...
    public void executeWorkItem(WorkItem workItem, WorkItemManager manager) {
        try {
            ...
            Map parameters = (Map) workItem.getParameter("parameters");
            JsonNode input = json.convertValue(parameters, JsonNode.class);
            ...
            Map result = Collections.singletonMap("result", json.readValue(invoke.getPayload().array(), Map.class));
